...
#if defined STM32WB55xx || defined STM32F429xx
...
#define FLASH_STATUS_ALL_FLAGS                                            \
  (FLASH_SR_RDERR | FLASH_SR_PGSERR | FLASH_SR_PGAERR | \
...
secbool flash_write_byte(uint16_t sector, uint32_t offset, uint8_t data) {
  ...
  if (HAL_OK != HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, address, data)) {
    return secfalse;
  }
