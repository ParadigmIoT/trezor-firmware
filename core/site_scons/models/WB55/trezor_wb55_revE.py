from __future__ import annotations

from .. import get_hw_model_as_number
from ..stm32wb55_common import stm32wb55_common_files


def configure(
    env: dict,
    features_wanted: list[str],
    defines: list[str | tuple[str, str]],
    sources: list[str],
    paths: list[str],
) -> list[str]:
    features_available: list[str] = []
    board = "WB55/boards/trezor_wb55_revE.h"
    display = "st7789v.c"
    hw_model = get_hw_model_as_number("WB55")
    hw_revision = 0
    features_available.append("disp_i8080_8bit_dw")
    features_available.append("framebuffer")
    defines += ["FRAMEBUFFER"]

    if "new_rendering" in features_wanted:
        features_available.append("xframebuffer")
        features_available.append("display_rgb565")
        defines += ["DISPLAY_RGB565"]
        defines += ["XFRAMEBUFFER"]
    defines += ["USE_RGB_COLORS=1"]

    mcu = "STM32WB55xx"
    linker_script = """embed/trezorhal/stm32wb55/linker/u58/{target}.ld"""

    stm32wb55_common_files(env, defines, sources, paths)

    env.get("ENV")[
        "CPU_ASFLAGS"
    ] = "-mthumb -mcpu=cortex-m33 -mfloat-abi=hard -mfpu=fpv5-sp-d16 "
    env.get("ENV")[
        "CPU_CCFLAGS"
    ] = "-mthumb -mcpu=cortex-m33 -mfloat-abi=hard -mfpu=fpv5-sp-d16 -mtune=cortex-m33 -mcmse "
    env.get("ENV")["RUST_TARGET"] = "thumbv8m.main-none-eabihf"

    defines += [mcu]
    defines += [f'TREZOR_BOARD=\\"{board}\\"']
    defines += [f"HW_MODEL={hw_model}"]
    defines += [f"HW_REVISION={hw_revision}"]

    if "new_rendering" in features_wanted:
        sources += ["embed/trezorhal/xdisplay_legacy.c"]
        sources += ["embed/trezorhal/stm32wb55/xdisplay/st-7789/display_fb.c"]
        sources += ["embed/trezorhal/stm32wb55/xdisplay/st-7789/display_driver.c"]
        sources += ["embed/trezorhal/stm32wb55/xdisplay/st-7789/display_io.c"]
        sources += ["embed/trezorhal/stm32wb55/xdisplay/st-7789/display_panel.c"]
        sources += [
            "embed/trezorhal/stm32wb55/xdisplay/st-7789/panels/lx154a2482.c",
        ]
    else:
        sources += [f"embed/trezorhal/stm32wb55/displays/{display}"]
        sources += [
            "embed/trezorhal/stm32wb55/displays/panels/lx154a2482.c",
        ]

    sources += ["embed/trezorhal/stm32wb55/backlight_pwm.c"]
    features_available.append("backlight")
    defines += ["USE_BACKLIGHT=1"]

    env_constraints = env.get("CONSTRAINTS")
    if not (env_constraints and "limited_util_s" in env_constraints):
        sources += ["embed/trezorhal/stm32wb55/bg_copy.c"]

    if "input" in features_wanted:
        sources += ["embed/trezorhal/stm32wb55/i2c_bus.c"]
        sources += ["embed/trezorhal/stm32wb55/touch/ft6x36.c"]
        sources += ["embed/trezorhal/stm32wb55/touch/panels/lx154a2422cpt23.c"]
        features_available.append("touch")
    defines += ["USE_TOUCH=1"]
    defines += ["USE_I2C=1"]

    if "haptic" in features_wanted:
        sources += [
            "embed/trezorhal/stm32wb55/haptic/drv2625/drv2625.c",
        ]
        sources += [
            "vendor/stm32wb55xx_hal_driver/Src/stm32wb55xx_hal_tim.c",
            "vendor/stm32wb55xx_hal_driver/Src/stm32wb55xx_hal_tim_ex.c",
        ]
        features_available.append("haptic")
    defines += ["USE_HAPTIC=1"]

    if "sd_card" in features_wanted:
        sources += ["embed/trezorhal/stm32wb55/sdcard.c"]
        sources += ["embed/extmod/modtrezorio/ff.c"]
        sources += ["embed/extmod/modtrezorio/ffunicode.c"]
        sources += ["vendor/stm32wb55xx_hal_driver/Src/stm32wb55xx_hal_sd.c"]
        sources += ["vendor/stm32wb55xx_hal_driver/Src/stm32wb55xx_ll_sdmmc.c"]
        features_available.append("sd_card")
    defines += ["USE_SD_CARD=1"]

    if "sbu" in features_wanted:
        sources += ["embed/trezorhal/stm32wb55/sbu.c"]
        features_available.append("sbu")
    defines += ["USE_SBU=1"]

    if "usb" in features_wanted:
        sources += [
            "embed/trezorhal/stm32wb55/usb/usb_class_hid.c",
            "embed/trezorhal/stm32wb55/usb/usb_class_vcp.c",
            "embed/trezorhal/stm32wb55/usb/usb_class_webusb.c",
            "embed/trezorhal/stm32wb55/usb/usb.c",
            "embed/trezorhal/stm32wb55/usb/usbd_conf.c",
            "embed/trezorhal/stm32wb55/usb/usbd_core.c",
            "embed/trezorhal/stm32wb55/usb/usbd_ctlreq.c",
            "embed/trezorhal/stm32wb55/usb/usbd_ioreq.c",
            "vendor/stm32wb55xx_hal_driver/Src/stm32wb55xx_ll_usb.c",
        ]
        features_available.append("usb")

    if "dma2d" in features_wanted:
        defines += ["USE_DMA2D"]
        if "new_rendering" in features_wanted:
            sources += ["embed/trezorhal/stm32wb55/dma2d_bitblt.c"]
        else:
            sources += ["embed/trezorhal/stm32wb55/dma2d.c"]
        features_available.append("dma2d")

    if "optiga" in features_wanted:
        sources += ["embed/trezorhal/stm32wb55/optiga_hal.c"]
        sources += ["embed/trezorhal/optiga/optiga.c"]
        sources += ["embed/trezorhal/optiga/optiga_commands.c"]
        sources += ["embed/trezorhal/optiga/optiga_transport.c"]
        sources += ["vendor/trezor-crypto/hash_to_curve.c"]
        features_available.append("optiga")
    defines += ["USE_OPTIGA=1"]

    defines += [
        "USE_HASH_PROCESSOR=1",
        "USE_STORAGE_HWKEY=1",
        "USE_TAMPER=1",
        "USE_FLASH_BURST=1",
        "USE_RESET_TO_BOOT=1",
        "USE_OEM_KEYS_CHECK=1",
        "USE_PVD=1",
    ]

    env.get("ENV")["TREZOR_BOARD"] = board
    env.get("ENV")["MCU_TYPE"] = mcu
    env.get("ENV")["LINKER_SCRIPT"] = linker_script

    defs = env.get("CPPDEFINES_IMPLICIT")
    defs += ["__ARM_FEATURE_CMSE=3"]

    return features_available
